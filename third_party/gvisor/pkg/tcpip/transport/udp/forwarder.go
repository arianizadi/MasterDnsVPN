...
// Packet returns a reference to the first packet that triggered this request.
//
// The caller owns the returned reference and must release it with DecRef.
...
// Release releases the packet reference held by this request.
//
// Forwarder.HandlePacket increments the first packet reference before handing
// the request to the caller. Callers that drop the request or have already
// created an endpoint must release that reference.
