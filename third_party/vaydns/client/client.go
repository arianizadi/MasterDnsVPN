...
func (t *Tunnel) ListenAndServe(listenAddr string) error {
	return t.ListenAndServeContext(context.Background(), listenAddr)
}

// ListenAndServeContext is the embeddable variant of ListenAndServe. Cancelling
// ctx closes the local listener and tears down the active tunnel session.
func (t *Tunnel) ListenAndServeContext(ctx context.Context, listenAddr string) error {
	t.applyDefaults()
